...
// Workaround: Antlr4's BufferedTokenStream.prototype.getTokens() has a typo
// where it tries to call 'types.contains()' instead of 'types.includes()'.
...
// will generate warnings if any shadowed rules are discarded
function collate(rules) {
    const result = {};
    for (const rule of rules) {
        const rule_key = rule.type[2]?.yaml_key ?? rule.type[1];
        ...
        } else if (rule.type === RuleType.VPN) {
            rule.data = rule.data[0];
        ...
        if (Object.hasOwn(result, rule_key)) {
            ...
            switch (collation) {
                ...
                case JoinType.LAST_WINS:
                    // overwrite
                    // warn user
                    console.warn(`WARNING: pol2yaml ignoring dead code: ${rule.type[1]}:: ${JSON.stringify(result_rule)}`);
...
export default class PolicyParseTreeVisitor extends PolicyVisitor {
    ...
    visitVpn_rule(ctx) {
        const child_productions = this.visitChildren(ctx);
        const result = {};
        result.name = child_productions[3];

        if (child_productions.length > 4) {
            result.policy = child_productions[4];
        }

        child_productions.splice(3, Infinity, result);
        return child_productions;
    }
...
export class RuleType extends Enum {
    ...
    static VPN = new RuleType("VPN", "vpn", { "join_type": JoinType.LAST_WINS, "no_collapse": true })
    ...
    static LOG_NAME = new RuleType("LOG_NAME", "log_name", { "join_type": JoinType.LAST_WINS, "yaml_key": "log-name" })
