...
export default class SanityCheck {
    ...
    async create_tmpdir() {
        ...
        this.paths.input_pol = path.join(tmpdir, 'input/pol/');
        this.paths.input_yaml = path.join(tmpdir, 'input/yaml/');
        this.paths.output_pol = path.join(tmpdir, 'output/pol/');
        this.paths.output_yaml = path.join(tmpdir, 'output/yaml/');
        await mkdir(this.paths.input_pol, { recursive: true });
        await mkdir(this.paths.input_yaml, { recursive: true });
        await mkdir(this.paths.output_pol, { recursive: true });
        await mkdir(this.paths.output_yaml, { recursive: true });

        this.state = SanityCheckState.READY;
    }

    async writeFile(rel_path, text) {
        const file_path = path.join(this.paths.input_pol, rel_path);
    ...
    async writeOutput(rel_path, yaml) {
        const file_path = path.join(this.paths.input_yaml, rel_path);
        await mkdir(path.dirname(file_path), { recursive: true });
    ...
    async runChecks({ definitions_directory, config_file }) {

        // First we want to run aclgen on the input/pol directory and write to output/pol
        let aclgen_argv = [
            '--base_directory', this.paths.input_pol,
            '--output_directory', this.paths.output_pol,
        ...
        aclgen_argv = [
            '--base_directory', this.paths.input_yaml,
            '--output_directory', this.paths.output_yaml,
            '--definitions_directory', definitions_directory,
        ];
        if (config_file) {
            aclgen_argv.push('--config_file', config_file);
        ...
        try {
            ...
            await execFile('diff', ['-r', this.paths.output_pol, this.paths.output_yaml]);
        } catch (e) {
            if (/^spawn/.test(e.syscall) && e.code == 'ENOENT') {
