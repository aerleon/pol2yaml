...
async function pol2yamlInner({
    ...
    for (const target of expanded_targets) {
        ...
        try {
            target.path_parts = path.parse(target.path)
            const extname = target.path_parts.ext;
            switch (extname) {
                case '.inc':
                case '.pol':
        ...
        // output phase
        {
            const output_path = path.join(output_directory, path.format({...target.path_parts, ext: '.yaml', base: ''}));
            ...
            if (sanity_checker) {
                sanity_checker.writeOutput(target.root, path.relative(
                    target.root === 'def' ? definitions_directory : base_directory,
                    path.format({...target.path_parts, ext: '.yaml', base: ''})),
                    target.yaml);
            }
        }
    }
